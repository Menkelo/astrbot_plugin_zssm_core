...
class LLMPlan:
    ...
    is_search: bool = False  # True=识别到在线搜索指令，拐到视觉模型处理（视觉模型自带联网能力）
...
class ZssmExplain(Star):
    ...
    @staticmethod
    def _decide_search(inline: str) -> bool:
        """识别显式在线搜索指令（如“搜索/查一下/联网”）。仅显式指令触发，不改写请求，
        由视觉模型（自带联网能力）直接处理。"""
        if not inline:
            return False
